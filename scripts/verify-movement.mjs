...
const CASES = [
  ...
  },
  {
    ...
    //
    // 좌표 주의: 런 종점의 x를 드리블 종점(=패스 출발점)보다 앞에 두면 안 된다.
    // 오프사이드 규칙 3번("리시버가 공보다 골문 쪽")에 걸려 패스가 확률과 무관하게
    // 전 시드 실패하고, 관찰할 재생 자체가 안 나온다. 예전에는 런 목표가 (92,44)라도
    // 가동 반경에 잘려 공 뒤에 멈췄는데, 능력치를 자체 산출로 바꾸며 황희찬이 빨라지자
    // (가속 15→17) 클램프 종점이 공을 추월해 버렸다. 그래서 둘 다 x=86으로 맞춰
    // "공과 나란한 높이로 횡 침투"라는, 온사이드가 구조적으로 보장되는 그림으로 둔다.
    name: '런 → 수신 → 드리블 → 슛 (같은 선수)',
    chainActs: [
      { type: 'dribble', actorId: 'kor_07', to: D(86, 36) },
      { type: 'pass', receiverId: 'kor_11' },
      { type: 'dribble', to: D(100, 42) },
      { type: 'shot', receiverId: 'GOAL', to: D(119, 40) },
    ],
    runs: [{ id: 'kor_11', to: D(86, 44), afterIndex: 0 }],
  },
  {
