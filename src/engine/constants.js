...
//
// v2 → v2.1 변경 요약:
//  - 스탯 스케일: 0~100 → 1~20 (보고서 부록 A 어댑터 교체)
...
export const K = {
  ...
  // 스탯 어댑터 — 1~20 스케일 (부록 A: norm(v) = 0.3 + 0.7·v/20, 출력 [0.335, 1.0])
  STAT: {
    FM_MAX: 20,
    MID: 0.7, // 스킬·수비 항 공통 중심점 (스탯 ≈ 11.4 = 월드컵 출전급 평균)
  ...
  // PRESS_FLOOR: 경합 게이트에 넣기 전 압박에서 빼는 바닥값 (pressure 단위 e^(−d/R_DEF)).
  //   0.01 = 약 14m — 그보다 멀면 압박이 **정확히 0**이 되어 "뺏을 사람이 없으면
  //   실패도 없다"는 규칙이 성립한다. 빼기만 하고 자르지 않으므로 계단이 생기지 않는다.
  //   예전에는 ATTRIBUTION_MIN(0.15 ≈ 5.7m)으로 잘라서, 경로에서 5m면 82.5%인데
  //   6m면 갑자기 100%가 되는 절벽이 있었다.
  DRIB: { Z0: 2.2, B_SKILL: 1.2, B_LEN: -0.06, B_DEF: -2.2, R_DEF: 3.0, B_BODY: 0.8, B_SURROUND: -0.9, PRESS_FLOOR: 0.01 },
  ...
  // 매치엔진들의 통상 구조: 수비는 틱마다 조널/마킹/압박 목표를 재계산하고,
