...
// names: { a: 액션 주체, b: 패스 수신자, d: 차단한 수비수, team: 공격 팀 이름 }
//
// team은 화면에 뜨는 나라 이름이다. 팀 이름을 멘트에 박아 두면 다른 경기에서 그대로
// 새어 나온다 — 실제로 스페인-포르투갈전에서 "대한민국이 앞서갑니다"가 떴다.
// 스코어도 단정하지 않는다: 이스터에그 장면 7개 중 잉글랜드-아르헨티나는 0:1로
// 지고 있는 상황이라 "앞서갑니다"가 성립하지 않는다. 멘트는 스코어를 모른다.
const POOL = {
  dribble: [
    ({ a }) => `${ga(a)} 공을 몰고 전진합니다!`,
    ({ a }) => `${a}, 과감하게 치고 나갑니다!`,
  ...
  goal: [
    ({ a }) => `골!! 골!! 골!! ${ui(a)} 득점입니다!!`,
    ({ a }) => `들어갔습니다!! ${ui(a)} 극적인 골!!`,
    ({ a, team }) => `골망이 흔들립니다!! ${a}!!${team ? ` ${team} 벤치가 모두 뛰쳐나옵니다!!` : ''}`,
  ],
  shotMiss: [
  ...
  // 선방 — 골키퍼가 슛선에서 막아낸 경우. shotBlocked(필드 수비수의 몸막이)와 구분한다.
  shotSaved: [
    ({ a, d }) => `${ga(d)} ${ui(a)} 슛을 쳐냅니다!`,
    ({ a, d }) => `${ui(a)} 슈팅 — ${ga(d)} 몸을 날려 걷어냅니다!`,
    ({ a, d }) => `${a}, 골문을 노렸지만 ${ui(d)} 손끝에 걸립니다!`,
  ],
  passCut: [
